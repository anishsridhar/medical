var INVN = 7651; // 256 * 7651 = 1 mod q(7681)
...
//--------------------------------------------------------------------------------------
//random function defn and mash function
function Alea() {
...
var n = 256; //plain text size
var q = 7681; // constant value for kyber, small prime 


function convertToBinary(array) {
    var size = array.length;
    while (!Number.isInteger(Math.sqrt(size))) {
        size = size - 1;
    }
    size = Math.sqrt(size);
    const binaryMatrix = [];
    ...
    for (let i = 0; i < size; i++) {
        ...
        for (let j = 0; j < size; j++) {
            // Set the corresponding value in the binary matrix based on the array element
            binaryMatrix[i][j] = array[index] > 0 ? 1 : 0;
            index++;
        }
    }
...
function generateMatKey() {
    var genmat = c100 + c101 + c102 + c200;
    genmat = convertToBinary(genmat);
    return genmat;
}
...
function testkyber() {
    var n = 256; //plain text size
    var q = 7681; // constant value for kyber, this is required to enable the fast NTT-based multiplication
    ...
    var kv = v00.toString();
    var mat = c100 + c101 + c102 + c200;
    var mat1 = convertToBinary(mat);
    //console.log(mat);
    //console.log(mat1);
    //console.log(c100,c101,c102,c200);
